 ...
 * Renders structured flight data as rich visual cards in the chat.
 */

const FlightCardRenderer = {
    /**
     * Render a single flight as an HTML card.
     * @param {Object} flight — normalized flight object from API
     * @param {number} rank — position in results (1-based)
     * @returns {string} HTML string
     */
    renderCard(flight, rank = 0) {
        ...
        const badges = [];
        if (isBest) badges.push('<span class="flight-badge flight-badge-best">⭐ Best</span>');
        if (flight.often_delayed) badges.push('<span class="flight-badge flight-badge-delayed">⚠️ Often Delayed</span>');
        if (flight.stops === 0) badges.push('<span class="flight-badge flight-badge-direct">✈ Direct</span>');
        ...
        <div class="flight-card ${bestClass}">
            <div class="flight-card-header">
                <div>
                    ${rank ? `<span style="color:var(--text-muted);font-size:0.8rem">#${rank}</span> ` : ''}
                    <span class="flight-airline">${flight.airlines.join(', ')}</span>
                    <span style="color:var(--text-muted);font-size:0.8rem"> ${flight.flight_numbers.join(', ')}</span>
                </div>
            ...
            <div class="flight-meta">
                ...
                ${flight.carbon_g ? `<span>🌱 ${(flight.carbon_g / 1000).toFixed(0)}kg CO₂</span>` : ''}
            </div>
        </div>`;
    },

    /**
     * Render multiple flights as a list of cards.
     * @param {Array} flights — array of flight objects
     * @param {number} maxCards — maximum cards to render
     * @returns {string} HTML string
     */
    renderCards(flights, maxCards = 10) {
        ...
        if (flights.length > maxCards) {
            html += `<p style="color:var(--text-muted);font-size:0.85rem;margin-top:var(--space-sm)">
                ...và ${flights.length - maxCards} kết quả khác
            </p>`;
        }
    ...
    /** Format time string "2026-06-20 08:40" → "08:40" */
    _formatTime(timeStr) {
        if (!timeStr) return '--:--';
        const parts = timeStr.split(' ');
        return parts.length >= 2 ? parts[1].substring(0, 5) : timeStr.substring(0, 5);
    },

    /** Format minutes → "2h 10m" */
